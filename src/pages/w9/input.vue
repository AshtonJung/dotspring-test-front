<template>
  <section class="mx-auto w-full min-h-screen bg-slate-50">
    <!-- 등록 중 오버레이 -->
    <div
      v-if="isSubmitting"
        ...
        <div v-show="guideOpen" class="mt-4 space-y-4 text-sm leading-6 text-slate-700">
          <div>
            ...
            <div class="mt-1 text-slate-600">
              완료 예정일 Business Day 기준 (FBM + 1일, SPD + 2일, LTL + 3일 이내 진행 목표)
            </div>
  ...
  <div class="grid gap-3 sm:grid-cols-3">
    <div class="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
      <div class="text-xs font-semibold text-slate-500">회사명</div>
      <input
    ...
    <div class="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
      <div class="text-xs font-semibold text-slate-500">담당자 (수정가능)</div>
      <input v-model="form.requesterName" type="text" class="input mt-2" />
    </div>

    <div class="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
      <div class="text-xs font-semibold text-slate-500">이메일 (수정가능)</div>
      <input v-model="form.requesterEmail" type="email" class="input mt-2" />
    </div>
  </div>
</div>
      </div>
      ...
      <form @submit.prevent="onSubmit" class="flex flex-col gap-5">
        ...
        <div class="card">
          <div class="card-title">
            1. 업무 유형 <span class="text-rose-600">*</span>
          </div>
          <div class="card-body">
            <div class="relative">
              <select
        ...
        <div class="card">
          <div class="card-title">
            3. 요청 예정일 (D+1 이상) <span class="text-rose-600">*</span>
          </div>
          <div class="card-body">
            <input
...
const validate = () => {
  ...
  if (!form.requesterName?.trim()) return "담당자를 입력해 주세요.";
  if (!form.requesterEmail?.trim()) return "이메일을 입력해 주세요.";
  if (!form.requestTypeCode) return "업무 유형을 선택해 주세요.";
  if (!getPlainText(form.requestDetail)) return "요청 내용을 입력해 주세요.";
  if (!form.dueDate) return "요청 예정일을 선택해 주세요.";
  if (form.dueDate < minDate.value) {
    return `요청 예정일은 ${minDate.value} 이후로 선택해 주세요.`;
  }
...
const uploadEditorImage = async (file) => {
  ...
  if (!putRes.ok) {
    const text = await putRes.text().catch(() => "");
    throw new Error(`이미지 업로드 실패: ${putRes.status} ${text}`);
  }

  if (!key) {
    throw new Error("업로드 key 없음");
  }
...
onMounted(async () => {
  ...
  form.companyName = requesterCompany.value || "";
  form.requesterName = requesterName.value || "";
  form.requesterEmail = requesterEmail.value || "";
